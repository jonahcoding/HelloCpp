...
> 存放函数内的普通局部变量、形参和函数返回值，函数结束即释放。
>
> 效率高（栈内存分配运算置于处理器的指令集中），内存有限。

②堆区

> 由程序员使用malloc()等函数申请，使用free()释放。

③自由存储区

> 通过new申请的内存，对应使用delete释放。未使用delete释放的内存，程序结束后由OS回收。

④全局/静态存储区

> 存放全局变量和静态变量，申请的内存在程序全部结束后释放。

⑤常量存储区

> 程序运行期间不能被改变的常量，如：10，字符串常量"abc"，数组的名字等。
