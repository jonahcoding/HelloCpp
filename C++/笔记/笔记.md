...
C++内存划分为五个区，分别是：栈、堆、全局/静态存储区、常量存储区、代码区。
...
#### 1.1.2 全局/静态存储区扩展（C语言下）

全局/静态存储区划分为**已初始化区（data段）**和**未初始化区（bss段）**

- 已初始化区：存放***初始化值非0***的全局变量和静态变量。
...
int a = 0;                      //全局初始化区（C语言下区分）
char* p1;                       //全局未初始化区（C语言下区分）
...
- 通过new申请的内存属于自由存储区，对应使用delete释放。未释放的内存，程序结束后由OS回收。

- 多数（几乎所有）编译器的new/delete以malloc/free为基础来实现。
